...
public abstract class CModel extends CComponent/* implements IteratorAggregate, ArrayAccess*/
{
     ...
     */
    abstract public List<String> attributeNames();
     ...
     */
    public List getValidators(String attribute/*=null*/)
    {
        if(this._validators == null)
            this._validators = this.createValidators();
    ...
    {
        for (this.getValidators($attribute) as $validator)
        {
    ...
    {
        List<String> attributes = this.getSafeAttributeNames();
     ...
     */
    public Map<String,Object> getErrors(String attribute/*=null*/)
    {
        if (attribute==null)
            return this._errors;
        else
            return (Map<String, Object>) (isset(this._errors, attribute) ? this._errors.get(attribute) : array());
    }
    ...
    {
        return isset(this._errors, attribute) ? reset(this._errors.get(attribute)) : null;
    }
...
//        this._errors[$attribute][]=$error;
    ...
    {
        return ucwords(trim(strtolower(str_replace(Arrays.asList("-","_",".")," ",
     ...
     */
    public Map<String, Object> getAttributes(List names/*=null*/)
    {
        values=array();
        for(Object name : this.attributeNames()/* as $name*/)
     ...
     * @see attributeNames
     */
    public void setAttributes(Map<String, Object> values, boolean safeOnly/*=true*/)
    {
        if(!is_array(values))
            return;
        attributes = array_flip(safeOnly
                ? this.getSafeAttributeNames()
                        : this.attributeNames());
        for (Entry<String, Object> ent : values.entrySet()/* as $name=>$value*/)
        {
     ...
     * @since 1.1.3
     */
    public void unsetAttributes(List<String> names/*=null*/)
    {
        if(names==null)
     ...
     * @since 1.1.1
     */
    public void onUnsafeAttribute(String name, Object value)
    {
        if(YII_DEBUG)
            Yii.log(Yii.t("yii",
                    "Failed to set unsafe attribute \"{attribute}\" of \"{class}\".",
                    array("{attribute}", name, "{class}" ,get_class(this))),
     ...
     */
    public List<String> getSafeAttributeNames()
    {
        attributes = array();
        List<String> unsafe = new ArrayList<String>();
        for (Object validator : this.getValidators()/* as $validator*/)
        {
            if(!validator.safe)
            {
                for(String name : validator.attributes/* as $name*/)
                    unsafe.add(name)/*[]=$name*/;
            }
            else
            {
                for(String name : validator.attributes/* as $name*/)
        ...
        for (String name : unsafe/* as $name*/)
     ...
     */
    public void offsetSet(int offset, Object item)
    {
        $this->$offset=$item;
    }
