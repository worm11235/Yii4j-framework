...
class CDateFormatter extends CComponent
{
     ...
     */
    private static Map<String, Object> _formatters = array(
            "G", "formatEra",
            "y", "formatYear",
            "M", "formatMonth",
            "L", "formatMonth",
     ...
     */
    public CDateFormatter(Object locale)
    {
        if(is_string(locale))
            ...
            this._locale=locale;
             ...
             public String format(String pattern, String time)
             {
                 if(time==null)
                     return null;
                 long $time =  0;
                 if(ctype_digit(time) || (time.charAt(0) == '-' && ctype_digit(substr(time, 1))))
                     $time = Integer.parseInt(time);
                 else
                     $time = strtotime(time);
                 
                 return format(pattern, $time);
             }
              ...
              */
             public String formatDateTime(String timestamp, String dateWidth/*="medium"*/, String timeWidth/*="medium"*/)
             {
                 if(!empty(dateWidth))
             ...
             protected Stack parseFormat(String pattern)
             {
                 ...
                 for(int i=0; i < n; ++i)
                 {
                     ...
                     else
                     {
                         ...
                         char[] ps = new char[j-i];
                         Arrays.fill(ps, c);
                         String p = new String(ps);
                         if(isset(this._formatters, c))
                             tokens.push = array(this._formatters.get(c), p);
                         else
                             tokens.push(p);
                         i = j - 1;
                     }
              ...
              */
             protected String formatYear(String pattern, Map<String, Object> date)
             {
              ...
              */
             protected String formatMonth(String pattern, Map<String, Object> date)
             {
                 $month = date.get("mon");
                 switch(pattern)
                 {
                     ...
                     default:
                         throw new CException(Yii::t("yii","The pattern for month must be \"M\", \"MM\", \"MMM\", \"MMMM\", \"L\","
                                 + " \"LL\", \"LLL\" or \"LLLL\"."));
                 }
             }
              ...
              */
             protected String formatDay(String pattern, Map<String, Object> date)
             {
                 $day=$date["mday"];
                 if($pattern==="d")
                     return $day;
