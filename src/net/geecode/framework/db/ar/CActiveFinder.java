...
import net.geecode.framework.db.ar.CJoinQuery.CStatElement;
...
class CActiveFinder extends CComponent
{
     ...
     */
    private void buildJoinTree(CJoinElement parent, Object with, Map options/*=null*/)
    {
...
class CJoinElement
{
     ...
     */
    public Map children=array();
    /**
     * @var array list of stat elements
     */
    public Map stats=array();
    /**
            ...
            CJoinElement parent/*=null*/, int id/*=0*/)
    {
        ...
        if(this.model.getDbConnection().getDriverName()==="oci")  // Issue 482
            ...
            $prefix="t".$id."_c";
        for($table.getColumnNames() as $key=>$name)
        {
            $alias=$prefix.$key;
    ...
    public void find(CDbCriteria criteria/*=null*/)
    {
        ...
        for(this.children as $child) // find recursively
            $child.find();
...
class CJoinQuery
{
     ...
     */
    public List selects = new ArrayList();
    /**
    ...
    public CDbCommand createCommand(CDbCommandBuilder builder)
    {
        ...
        conditions = array();
        for (this.conditions as $condition)
            if (condition != "")
